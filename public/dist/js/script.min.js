...
const ClientCart = {
    ...
    data: [],
    Add: function (sessionid, produto_id, produto_titulo, qtde, preco) 
    {
        ...
        if(ClientCart.SizeOf()>0)
        {
            for(var i = 0; i< ClientCart.SizeOf(); i++) {
                if(ClientCart.data[i].pid !== produto_id){
                    ClientCart.data.push( { sessionid: sessionid, pid : produto_id , nome: produto_titulo, qtde : qtde, preco : parseFloat(preco).toFixed(2) } )
                    break;
                }
            }
        }
