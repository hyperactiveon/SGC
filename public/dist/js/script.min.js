...
const WishList = {
    ...
    data: [],
    Add: function (produto_id) 
    {
        if (WishList.data.indexOf(produto_id) === -1) {
            try {
                WishList.data.push(produto_id)
            } catch(e) {
                try {
                    var test = produto_id.split("-")
                    WishList.data.push("-"+test[1])
                } catch(e) {}
            }
            Armazenamento.Set("whish_list",WishList.data)
        }
        document.getElementById("badge-tot-whish-list1").textContent = WishList.SizeOf()
        if(document.getElementById("badge-tot-whish-list2")){
            document.getElementById("badge-tot-whish-list2").textContent = WishList.SizeOf()
        }
    },
    Del: function (produto_id){
        try {
            WishList.data.splice(WishList.data.indexOf(produto_id), 1);
        } catch(e) {
            var test = produto_id.split("-");
            try {
                WishList.data.splice(WishList.data.indexOf(test[0]+"-"+test[1]), 1);
            } catch(e) {
                WishList.data.splice(WishList.data.indexOf("-"+test[1]), 1);
            }
        }
        Armazenamento.Set("whish_list",WishList.data)
        document.getElementById("badge-tot-whish-list1").textContent = WishList.SizeOf()
    ...
    SetItens: function ()
    {
        ...
        if(localStorage.whish_list!=undefined) {
            ...
            var test = document.querySelectorAll('.wishlist.atw-demo')[0].id.split("-");
            for(var i=0; i<WishList.SizeOf(); i++) {
                try {
                    document.getElementById(WishList.data[i]).setAttribute("class","wishlist atw-demo active")
                } catch(e) {
                    var test2 = WishList.data[i].split("-")
                    try {
                        document.getElementById("-"+test2[1]).setAttribute("class","wishlist atw-demo active")
                    } catch(e) {
                        document.getElementById(test[0]+"-"+test2[1]).setAttribute("class","wishlist atw-demo active")
                    }
                }
            }
            document.getElementById("badge-tot-whish-list1").textContent = WishList.SizeOf()
...
const ClientCart = {
    ...
    {
        var test = produto_id.split("-")
        if (ClientCart.data.indexOf(test[1]) === -1) {
            ClientCart.data.push(test[1])
            Armazenamento.Set("cart_list",ClientCart.data)
        }

        document.getElementById("badge-tot-cart-list1").textContent = ClientCart.SizeOf()
        if(document.getElementById("badge-tot-cart-list2")){
            document.getElementById("badge-tot-cart-list2").textContent = ClientCart.SizeOf()
        }
    },
    Del: function (produto_id){
        try {
            ClientCart.data.splice(ClientCart.data.indexOf(produto_id), 1);
        } catch(e) {
            var test = produto_id.split("-");
            try {
                ClientCart.data.splice(ClientCart.data.indexOf(test[0]+"-"+test[1]), 1);
            } catch(e) {
                ClientCart.data.splice(ClientCart.data.indexOf("-"+test[1]), 1);
            }
        }
        Armazenamento.Set("cart_list",ClientCart.data)
        document.getElementById("badge-tot-cart-list1").textContent = ClientCart.SizeOf()
